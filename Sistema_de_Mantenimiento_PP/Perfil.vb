...
Public Class Perfil
    ...
    Private Sub Perfil_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        'TODO: esta línea de código carga datos en la tabla 'PROYECTO_DB_PARTIDOS_POLITICOSDataSet4.PERFIL' Puede moverla o quitarla según sea necesario.
        Me.PERFILTableAdapter.Fill(Me.PROYECTO_DB_PARTIDOS_POLITICOSDataSet4.PERFIL)
    ...
    Private Sub Botinsertar_Click(sender As Object, e As EventArgs) Handles Botinsertar.Click
        ...
        Try
            ...
            Dim cmdTipoid As New SqlCommand("INSERT INTO PERFIL(CODIGO_ID,DESCRIPCION)
    ...
    Private Sub Botconsultar_Click(sender As Object, e As EventArgs) Handles Botconsultar.Click
        ...
        Dim dt As DataTable = conexionManager.busca_codigo_quinto(vtipoid)

        If f = 1 OrElse dt.Rows.Count = 0 Then
            MsgBox("Codigo de perfil, no registrado")
            Txt_tipoid.Focus()
            Return
        Else
            k = 1
    ...
    Private Sub Botmodificar_Click(sender As Object, e As EventArgs) Handles Botmodificar.Click
        ...
        Try
            cn = conexionManager.Obtener_Conexion()
            transaccion = cn.BeginTransaction()
            Dim cmdTipoidm As New SqlCommand("UPDATE PERFIL SET DESCRIPCION=@DESCRIPCION WHERE CODIGO_ID=@CODIGO_ID", cn, transaccion)
    ...
    Private Sub Boteliminar_Click(sender As Object, e As EventArgs) Handles Boteliminar.Click
        ...
        Try
            If MessageBox.Show("¿Está seguro de eliminar el registro en la base de datos?", "Confirmacion", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
                cn = conexionManager.Obtener_Conexion()
                transaccion = cn.BeginTransaction()
                Dim cmdtipoidm As New SqlCommand("DELETE FROM PERFIL WHERE CODIGO_ID=@CODIGO_ID", cn, transaccion)
                cmdtipoidm.Parameters.AddWithValue("@CODIGO_ID", Txt_tipoid.Text)
                cmdtipoidm.ExecuteNonQuery()
